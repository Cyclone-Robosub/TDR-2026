...
==== Binocular Camera Enclosures=== Kill Switch 
...
[reed switch diagram here]

The decision for a mechanical reed switch allowed us to create a system where a U-shaped handle containing a epoxy coated neodymium magnet is pulled away from it's attraction to the magnetic reed switch, turning the robot off. The choice for handle's rounded U-shape stops external objects from catching onto it, while being large enough that it cannot be damaged significantly. Our magnet choice of neodymium also allows it to be powerful enough to stay on the robot, as well as resist corroding due to the epoxy.

[railing diagram here]

For the location of the kill switch system, we resolved to resting the mechanism on the acrylic tube in the middle. This position can be reached fairly evenly from every angle of the robot, while also having no obstruction of wires nearby. The kill switch system also includes a railing system to adjust for horizontal movements of the tube.
...
// to add later today: discussion on the location of the kill switch + use of the arm. explanation for why we added a railing. diagrams for both. long term implementation and functionality discussion. edits on previous paragraphs. try to tie everything to the objectives.
  A full design revision can be found in.
...
The decision for a mechanical reed switch allowed us to create a system where a U-shaped handle containing a neodymium magnet is pulled away from it's attraction to the magnetic reed switch, turning the robot off. The choice for handle's rounded U-shape stops external objects from catching onto it, while being large enough that it cannot be damaged significantly.

[diagram here]

The location of the kill switch system also had to be changed to arch over the center of the acrylic tube as this is the point where the kill switch can relatively be grabbed equally from every angle. Along with that, we included a railing system so that the kill switch's location can be adjusted for buoyancy.

In the long term, some adjustments can be made such that the whole system is a lot more compact. But the system itself will make it will make it infinitely easier to turn off the robot in case of any danger.
