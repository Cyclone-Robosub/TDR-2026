...
#let ieee(
  ...
  set document(title: "Cyclone RoboSub @ UC Davis - 2025 Technical Design Report ", author: authors.map(author => author.name))
  ...
  set page(
    columns: 2,
    header: context{[ Cyclone RoboSub \@ UC Davis #h(1fr) #counter(page).display()]},
    // numbering: "1",
    paper: paper-size,
  ...
  set std.bibliography(title: text(10pt)[References], style: "ieee", full: true)
  ...
  place(
    ...
    clearance: 30pt,
    {
      ...
      // for i in range(calc.ceil(authors.len() / num-across)) {
      //   let end = calc.min((i + 1) * num-across, authors.len())
      //   let is-last = authors.len() == end
      //   let slice = authors.slice(i * num-across, end)
